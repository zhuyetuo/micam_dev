...
**延迟高（1-2 秒）怎么办：** 大部分来自 FFmpeg/OpenCV 默认的内部缓冲，加上 HEVC 本身 GOP/B 帧重排序、以及分辨率越高摄像头编码转发越慢。加 `--low-latency` 用后台线程只处理最新一帧，丢掉排队的旧帧：

```bash
python scripts/capture_frame.py --stream cam0 --low-latency
```

代价是可能跳过中间某些帧（对实时预览/检测通常没影响）；如果对"不漏帧"有硬性要求（比如录像分析），就不要加这个参数。降低 `subtype`（分辨率）画质档位通常也能减小延迟。
